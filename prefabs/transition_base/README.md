...
`shaders/sh_transition/fragment.fsh`, plus `shader.toml` when they have knobs.
...
`{ amplitude: 60, speed: 2.0 }`. Unspecified knobs fall back to the defaults
in the effect's `shaders/sh_transition/shader.toml`, which lists exactly what
a given effect exposes; the editor previews the transition with those knobs.
The base fade effect has no knobs.
